...
| 替代关系 | 2026-09-16 起研究部分由 [ADR-0002](../adr/ADR-0002-语义意图与多源研究闭环.md)局部替代；原 P0～P5 批准事实和其他组件不变 |
...
> **局部替代记录（2026-09-16）：** 项目负责人批准按三份实施计划逐阶段开发；[ADR-0002](../adr/ADR-0002-语义意图与多源研究闭环.md)仅替代本文第 8、16 节中“研究仅单一模型服务端搜索、不得引入 RSS/网页正文解析栈”的限制，准许经准入的免费来源及 feedparser、Trafilatura、dateparser、HTTPX/tzdata 离线依赖。以下旧正文保留为 2026-09-02 已批准历史基线，不再作为该局部 Agent 侧范围的禁止依据。真实来源启用、模型新费用、生产切换、共享 DDL 和 Java/UI 均未授权。
...
P0～P5 局部技术方向和跨阶段总体一致性评审均已确认，最终跨阶段裁决见[Agent 侧 P0～P5 总体技术架构设计](../superpowers/specs/2026-09-02-Agent侧P0-P5总体技术架构-设计.md)。2026-09-02 的原始冻结规则保留为历史；2026-09-16 起，ADR-0002 已解除其覆盖范围内 Agent 离线依赖准入和分阶段实现的冻结。真实来源/API 启用、共享数据库连接与 DDL、生产 PoC、部署及范围外 P1 工作仍未因该局部授权开放。
