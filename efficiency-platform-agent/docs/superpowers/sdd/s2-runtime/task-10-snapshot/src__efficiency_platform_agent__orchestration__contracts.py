...
class GraphProgram(Protocol):
    async def invoke(self, initial_state: AgentGraphState, config: Mapping[str, JsonValue]) -> Mapping[str, JsonValue]:
        raise NotImplementedError

    async def resume(self, resume_value: JsonObject, config: Mapping[str, JsonValue]) -> Mapping[str, JsonValue]:
        raise NotImplementedError

    async def get_state(self, config: Mapping[str, JsonValue]) -> Mapping[str, JsonValue]:
        raise NotImplementedError
...
class CheckpointView:
    ...
    def __post_init__(self) -> None:
        for name, value in (("thread_id", self.thread_id), ("checkpoint_ns", self.checkpoint_ns), ("checkpoint_id", self.checkpoint_id)):
...
class GraphRegistration:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.graph_id, str) or not isinstance(self.graph_version, str):
...
class GraphRuntime:
    async def execute(self, selection: StrategySelection, initial_state: AgentGraphState) -> GraphExecutionResult:
        raise NotImplementedError

    def validate_resume(self, selection: StrategySelection, checkpoint: CheckpointView, resume_value: JsonObject) -> None:
        raise NotImplementedError

    async def resume(self, selection: StrategySelection, run_id: str, checkpoint_id: str, resume_value: JsonObject) -> GraphExecutionResult:
        raise NotImplementedError

    async def get_checkpoint(self, selection: StrategySelection, run_id: str) -> CheckpointView | None:
        raise NotImplementedError
