...
class InMemoryRunEventStore:
    ...
    _events: dict[tuple[str, str], list[RunEventRecord]] = field(default_factory=dict, init=False)
...
class FixedClock:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.epoch_ms, int) or isinstance(self.epoch_ms, bool) or self.epoch_ms < 0:
