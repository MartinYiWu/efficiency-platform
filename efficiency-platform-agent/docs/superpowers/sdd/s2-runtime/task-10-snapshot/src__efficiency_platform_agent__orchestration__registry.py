...
def validate_strategy_payload(schema_version: str, payload: JsonObject, allowed_keys: frozenset[str]) -> None:
    ...
    if len(json.dumps(plain(payload), ensure_ascii=False, separators=(",", ":")).encode("utf-8")) > 65536:
...
def build_default_registry(*, model_runtime=None, context_builder=None, tool_runtime=None, checkpointer=None) -> GraphRegistry:
    ...
    registry.register(GraphRegistration("direct", StrategyMode.DIRECT, "1.0.0", "s2:direct:1", "strategy.none/1", frozenset(), statuses, DirectGraphBuilder(model_runtime=model_runtime, context_builder=context_builder, tool_runtime=tool_runtime, checkpointer=checkpointer), _EmptyPayloadAdapter()))
    registry.register(GraphRegistration("workflow", StrategyMode.WORKFLOW, "1.0.0", "s2:workflow:1", "strategy.none/1", frozenset(), statuses, WorkflowGraphBuilder(model_runtime=model_runtime, context_builder=context_builder, tool_runtime=tool_runtime, checkpointer=checkpointer), _EmptyPayloadAdapter()))
    return registry
