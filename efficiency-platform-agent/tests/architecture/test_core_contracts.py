...
class CoreContractsTest(unittest.TestCase):
    ...
    def test_core_agent_does_not_depend_on_agent_runtime_implementations(self) -> None:
        source_path = SRC_ROOT / "efficiency_platform_agent/core/agent.py"
        tree = ast.parse(source_path.read_text(encoding="utf-8"))
        self.assertEqual(self._collect_core_agent_runtime_dependencies(tree), [])

    def test_core_agent_dependency_guard_rejects_relative_and_dynamic_import_bypasses(
        self,
    ) -> None:
        cases = {
            "from .. import agents": "agents",
            "from .. import providers": "providers",
            "import importlib\nimportlib.import_module('efficiency_platform_agent.agents')": (
                "efficiency_platform_agent.agents"
            ),
            "__import__('efficiency_platform_agent.providers')": (
                "efficiency_platform_agent.providers"
            ),
            "from importlib import import_module\nimport_module('efficiency_platform_agent.providers')": (
                "efficiency_platform_agent.providers"
            ),
        }

        for source, expected_dependency in cases.items():
            with self.subTest(source=source):
                dependencies = self._collect_core_agent_runtime_dependencies(
                    ast.parse(source)
                )
                self.assertIn(expected_dependency, dependencies)

    def _collect_core_agent_runtime_dependencies(self, tree: ast.AST) -> list[str]:
        ...
        forbidden_symbols = {"Provider", "Harness", "LangGraph"}
        dependencies: list[str] = []

        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if forbidden_module_segments.intersection(alias.name.split(".")):
                        dependencies.append(alias.name)
            elif isinstance(node, ast.ImportFrom):
                module_parts = (node.module or "").split(".")
                if forbidden_module_segments.intersection(module_parts):
                    dependencies.append(node.module or "<relative import>")
                for alias in node.names:
                    if forbidden_module_segments.intersection(alias.name.split(".")):
                        dependencies.append(alias.name)
                    if (
                        alias.name in forbidden_symbols
                        or alias.asname in forbidden_symbols
                    ):
                        dependencies.append(alias.asname or alias.name)
            elif isinstance(node, ast.Call) and self._is_dynamic_import_call(node):
                if node.args and isinstance(node.args[0], ast.Constant):
                    module_name = node.args[0].value
                    if isinstance(
                        module_name, str
                    ) and forbidden_module_segments.intersection(
                        module_name.lstrip(".").split(".")
                    ):
    ...
    def test_s2_runtime_core_does_not_import_upper_layers_or_frameworks(self) -> None:
        """S2 预算、运行事实和端口保持标准库及 core 内部依赖。"""
        forbidden = {
            "pydantic",
            "langgraph",
            "fastapi",
        ...
        for relative_path in (
            "efficiency_platform_agent/core/budget.py",
            "efficiency_platform_agent/core/runtime.py",
            "efficiency_platform_agent/core/runtime_ports.py",
        ):
            source_path = SRC_ROOT / relative_path
            tree = ast.parse(source_path.read_text(encoding="utf-8"))
            modules: list[str] = []
    ...
    def test_tool_contract_is_versioned_structured_and_bounded(self) -> None:
        ...
        resolved = {
            name: self._require_type(run_module, name) for name in required_types
        }
    ...
    def test_provider_contract_normalizes_messages_usage_and_errors(self) -> None:
        ...
        self.assertTrue(
            {"contract_version", "message", "usage", "error"}.issubset(result_fields)
        )
    ...
    def test_protocols_expose_descriptors_and_structured_requests(self) -> None:
        ...
        for name in ("StrategyExecutor", "AgentPlugin", "Tool", "ModelProvider"):
            ...
            with self.subTest(port=name):
                self.assertIs(
                    port.__annotations__.get("descriptor"), ExtensionDescriptor
                )
    ...
    def test_run_status_transition_table_covers_recovery_and_terminal_rules(
        self,
    ) -> None:
        run_module = importlib.import_module("efficiency_platform_agent.core.run")
