...
class AgentFactoryTest(unittest.TestCase):
    ...
    def test_create_wraps_builder_failure_without_leaking_original_details(
        self,
    ) -> None:
        registry = AgentRegistry()
    ...
    def test_create_wraps_invalid_run_contract_as_assembly_error(self) -> None:
        cases = (
            ("non_callable", 1),
            ("synchronous", lambda: None),
        )
        ...
        for case_name, run in cases:
            with self.subTest(case_name=case_name):
                ...
                registry.register(agent.spec, lambda plugin=plugin: plugin)

                with self.assertRaises(AgentAssemblyError) as caught:
                    AgentFactory(registry).create(agent.spec.agent_id)

                self.assertNotIn(case_name, str(caught.exception))
                self.assertIsInstance(
                    caught.exception.__cause__, AgentInstanceMismatchError
                )
