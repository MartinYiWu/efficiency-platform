...
from efficiency_platform_agent.harness.live_cluster_benchmark_v2 import (  # type: ignore[import-untyped]
...
async def execute_cluster_evaluation(
    *,
    ...
    if authorization is None:
        raise ValueError("CLUSTER_AUTHORIZATION_NOT_FOUND")
    now = datetime.now(UTC)
    if not authorization.approved_at <= now < authorization.expires_at:
        raise ValueError("CLUSTER_AUTHORIZATION_EXPIRED")
    if tenant_id not in authorization.allowed_tenant_ids:
        raise ValueError("CLUSTER_TENANT_NOT_AUTHORIZED")
    if "model_evaluation" not in authorization.allowed_actions:
        raise ValueError("CLUSTER_ACTION_NOT_AUTHORIZED")
    if approved_budget_microunits > authorization.max_budget_microunits:
        raise ValueError("CLUSTER_BUDGET_NOT_AUTHORIZED")
    ...
    provider_id = "deepseek_live_x04_cluster"
    registry.register(provider_id, provider)
    candidate = ModelCandidate(
        "deepseek-live-x04-cluster",
        provider_id,
        model_id,
        ModelTier.BALANCED,
        True,
    ...
    runtime = ModelRuntime(ModelPolicyRouter((candidate,)), registry)
    ...
    try:
        ...
        binding = PostgresLiveAcceptanceBudgetBinder(
            connection, "agent_runtime"
        ).bind(authorization, request, tenant_id=tenant_id)
        try:
            benchmark = build_frozen_cluster_benchmark(manifest_file)
            report = await evaluate_live_cluster_benchmark(
                benchmark,
                LiveModelClusterDecisionPort(runtime, binding),
                model_id=model_id,
            )
            snapshot = await binding.port.snapshot(binding.scope)
            return {
                **report,
                "execution_mode": "live-model-synthetic-benchmark",
                "external_io": True,
                "approved_budget_microunits": approved_budget_microunits,
                "authoritative_budget_usage": asdict(snapshot.used),
...
def main() -> int:
    ...
    parser.add_argument(
        "--manifest-file",
        type=Path,
        default=PROJECT_ROOT / "tests/fixtures/research_v2/corpus_manifest.json",
    )
    ...
    except Exception as exc:  # noqa: BLE001 - CLI 只暴露稳定异常类型
        result = {
        ...
        json.dumps(
            {
                ...
                "precision": (result.get("metrics") or {}).get("precision")
                if isinstance(result.get("metrics"), dict)
                else None,
                "recall": (result.get("metrics") or {}).get("recall")
                if isinstance(result.get("metrics"), dict)
                else None,
