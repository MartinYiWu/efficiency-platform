...
class ExecutionBudget:
    ...
    def __post_init__(self) -> None:
        ...
        _require_non_negative_int("max_cost_microunits", self.max_cost_microunits)
...
class ToolError:
    ...
    def __post_init__(self) -> None:
        ...
        if self.side_effect_status not in {"none", "possible", "confirmed"}:
            raise ValueError("side_effect_status must be none, possible, or confirmed")
...
class ProviderStreamChunk:
    ...
    delta: str = ""
    usage: ProviderUsage = field(default_factory=lambda: ProviderUsage(0, 0, 0, 0, 0))
...
class ProviderError:
    ...
    def __post_init__(self) -> None:
        ...
        _require_bool("retryable", self.retryable)
        _require_non_empty("safe_message", self.safe_message)
        _require_optional_non_empty("provider_request_id", self.provider_request_id)
...
def _validate_json_value(value: JsonValue) -> None:
    ...
    if isinstance(value, tuple):
        ...
        return
    raise TypeError("JSON values must use scalar, tuple, or JsonObject immutable forms")
