...
class RunFailure:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or not 1 <= len(self.safe_message) <= 512
        ):
    ...
    def transition(
        ...
        *,
        ...
        if target is RunStatus.SUCCEEDED and (output is None or failure is not None):
            raise ValueError("成功终态必须包含输出且不得包含失败")
        if target in {RunStatus.FAILED, RunStatus.TIMED_OUT} and (
            failure is None or output is not None
        ):
            raise ValueError("失败或超时终态必须包含错误且不得包含成功输出")
        if target not in {
            RunStatus.SUCCEEDED,
            RunStatus.FAILED,
            RunStatus.TIMED_OUT,
        } and (output is not None or failure is not None):
            raise ValueError("非终态不得携带最终输出或失败")
        next_budget = self.budget_state if budget_state is None else budget_state
        if (
            next_budget.started_at_epoch_ms != self.budget_state.started_at_epoch_ms
            or next_budget.deadline_epoch_ms != self.budget_state.deadline_epoch_ms
        ):
...
class RunEventRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _validate_event_payload(
            self.payload, strategy_event=self.event_type == "strategy_event"
        )
...
def _validate_event_payload(
    value: object, *, strategy_event: bool, allow_nested_keys: bool = False
) -> None:
    ...
    if isinstance(value, JsonObject):
        for key, item in value.items:
            ...
            if (
                strategy_event
                and not allow_nested_keys
                and key not in _STRATEGY_EVENT_KEYS
            ):
                raise ValueError("strategy_event payload 存在未声明字段")
            child_allows_arbitrary = allow_nested_keys or key in {
                "counts",
                "usage_delta",
            }
            _validate_event_payload(
