...
class AgentValidator:
    ...
    def validate_instance(self, spec: AgentSpec, plugin: AgentPlugin) -> None:
        ...
        if not callable(plugin.run):
            raise AgentInstanceMismatchError("Agent 实例的 run 必须可调用")
        if not inspect.iscoroutinefunction(plugin.run):
            raise AgentInstanceMismatchError("Agent 实例的 run 必须是异步函数")
        if not isinstance(plugin.descriptor, ExtensionDescriptor):
            raise AgentInstanceMismatchError(
                "Agent 实例的 descriptor 不满足 ExtensionDescriptor 契约"
            )
        if plugin.spec != spec:
        ...
        if mismatches:
            ...
            raise AgentInstanceMismatchError(f"Agent 实例与注册定义不一致: {fields}")
